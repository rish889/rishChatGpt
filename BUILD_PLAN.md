...
- DB schema: `conversations`, `messages` (SQLite via SQLAlchemy). `users` deferred to
  Phase 4 — no point in a users table nothing references until real auth exists.
...
- Implemented in `backend/db.py`, `backend/models.py`, `backend/main.py`, and
  `frontend/src/{App.tsx,Sidebar.tsx,api.ts}`.
